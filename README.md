...
Schema & Raw Data import:             
Created a dedicated schema for the project as "Netflix_titles", Imported the CSV into a raw table as "netflix_raw". Raw data remains unchanged throughout the project.
...
1.Missing values-Replaced NULLS or blank spaces with 'Unknown' where appropriate.   
2.Removed duplicates- Identified duplicates using ROW_NUMBER().     
3.Standardized text columns- Removed extra spaces using TRIM().       
4.Date formatting- Converted date_added to proper DATE format.  
5.Duration cleanup- Spilted duration into two columns as duration_season and duration_minutes.  
6.Removed extra columns like duration and row_num. Changed 'text' datatype of all columns to appropriate ones. 
